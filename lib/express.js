...
function express() {
  ...
  app.methods.forEach(function(method) {
    app.routes[method] = { matches: {} };
    app[method] = function(ourPath, callback) {
      app.routes[method].matches[ourPath] = callback;

      if (ourPath instanceof RegExp) {
        app.routes[method].matches.regex = ourPath;
      }
    };
  ...
  app.getParams = function(method, reqPath) {
    var params;
    for (ourPath in app.routes[method].matches) {
  ...
  app.handle = function(req, res) {
    ...
    p
      .then(function() {
        if (app.routes[reqMethod].matches[reqPath] || params || match) {
          if (params) {
            req.params = params;
            app.routes[reqMethod].matches[params.path](req, res);
          } else if (match) {
          } else {
            app.routes[reqMethod].matches[reqPath](req, res);
          }
        } else {
          res.statusCode = 404;
