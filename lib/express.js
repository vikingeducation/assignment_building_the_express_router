...
function express() {
  ...
  app.handle = function(req, res) {
    ...
    // check req.method, loop through routes that match that method
    // pass each of those routes to a function that also takes in the req.url
    // loop will break if we find a match

    for (route in app.routes[method]) {
      parser.compare(registeredRoute, reqPath);
    }
